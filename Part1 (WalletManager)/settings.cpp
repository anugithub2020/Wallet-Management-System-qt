...
void Settings::on_btnAddCat_clicked()
{
    ...
    QString newCat = QInputDialog::getText(this, tr("New"),
                        tr("Enter New Category:"), QLineEdit::Normal,"", &ok);
...
bool Settings::insertUser(QString username, QString password)
{
    ...
    qDebug() << "DEBUG: User inserted: " << inserted;
...
void Settings::on_btnAddUser_clicked()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if(username.isEmpty() || password.isEmpty())
            QMessageBox::warning(this, "Fields empty", "Enter username and password");
        else {
            ...
            if(ok)
                ...
                QMessageBox::warning(this, "Fail", "An error has occured. Please try again");
        }
    }
}
...
{
    qDebug() << "DEBUG: Inside on_btnEditUser_clicked";
    if(ui->tblView->selectionModel()->selectedRows().isEmpty())
        return;
    ...
    QSqlRecord oldRecord = tableModel->record(rowNumber);
    qDebug() << "DEBUG: RowNumber" << rowNumber;
...
{
    qDebug() << "DEBUG: do_Toggle_Buttons: " << on;
